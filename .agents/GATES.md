...
3. **本条原文有数学错误，已由附录 D §D6/§D7 更正，正确表述以附录 D 为准。**
   原文写作："prefix-reset U30 在四图上被重加权 U30 双轴支配（`103/1522` 对 `73/1516`）。这不
   改变 §32/§B28 '经验前沿扩展、目标未完成'的判决，只是说明它落在前沿的进攻端且不是该端最优点。"
   —— `1522 > 1516`，Z6-F 超车更多，**两点互不支配**。附录 C 的 §C2 第 1 条、§C3、§C5 中受此
   错误影响的类级推断同样以附录 D 为准。
