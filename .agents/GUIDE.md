...
**当前项目覆盖（2026-08-06用户决定）**：后续模型接受/否决只使用固定`Austin600`。
near400、hard子集和跨地图结果如被引用，只是机制/迁移诊断，没有独立否决权。上表仍是通用
实验设计语言；只有用户重新明确指定守门或泛化验收panel，才恢复多panel联合验收。
...
- 报告当前结果时完整给出Austin600；其他诊断panel只有在实验预注册包含它时才报告，且不得
  用诊断结果替代Austin600验收。
