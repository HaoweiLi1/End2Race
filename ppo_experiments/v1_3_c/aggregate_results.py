#!/usr/bin/env python3
"""Strict aggregation for the preregistered PPO V1.3-C experiment."""
...
EXPERIMENT_DIR = ROOT / "ppo_experiments" / "v1_3_c"
...
def summarize_training(seed: int) -> dict[str, Any]:
    run_dir = RUN_ROOT / f"v1_3_c_seed{seed}"
    files = {
...
def evaluation_log(seed: int | None = None) -> dict[str, str]:
    ...
    path = RUN_ROOT / "v1_3_c_logs" / filename
    if not path.is_file():
...
def build_report(results: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "V1.3-C changes only V1.3-A's steering likelihood from the singular atanh-squashed "
        "parameterization to a physical Gaussian whose action is clipped by the existing environment.",
        "",
        "## Training process",
        "",
...
def main() -> None:
    ...
    if precheck.get("status") != "PASS" or preregistration.get("status") != "PREREGISTERED":
        raise ValueError("V1.3-C precheck/preregistration is not locked")
    ...
    if not training:
        raise ValueError("No formal V1.3-C run exists")

    results: dict[str, Any] = {
        "schema_version": 1,
        "record": "PPO_V1_3_C_RESULTS",
    ...
    if any(status == "STOPPED_KL_GUARDRAIL" for status in statuses):
        ...
        baseline_path = EVAL_ROOT / "end2race_bc_v1_3_c_Austin" / "multiagents" / "results_multi.json"
    ...
    print(f"V1_3_C_VERDICT={results['final_verdict']}")
