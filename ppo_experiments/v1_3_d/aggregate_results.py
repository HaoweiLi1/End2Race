#!/usr/bin/env python3
"""Strict aggregation for the preregistered PPO V1.3-D experiment."""
...
EXPERIMENT_DIR = ROOT / "ppo_experiments" / "v1_3_d"
...
def summarize_training(seed: int) -> dict[str, Any]:
    run_dir = RUN_ROOT / f"v1_3_d_seed{seed}"
    files = {
    ...
    expected_updates = list(range(1, len(metrics) + 1))
    if [int(row["update"]) for row in metrics] != expected_updates:
        raise ValueError(f"Seed {seed} metrics are not a contiguous U1 prefix")
    for row in metrics:
        steps = int(row["actual_optimizer_steps"])
        if not 0 < steps <= 16:
            raise ValueError(f"Invalid optimizer step count at seed {seed} U{row['update']}")
        actor_delta = row["actor_delta_from_bc"]
        if actor_delta["gru"]["relative_rms_delta_from_previous"] <= 0.0:
            raise ValueError(f"No GRU update at seed {seed} U{row['update']}")
        if actor_delta["output_layer"]["relative_rms_delta_from_previous"] <= 0.0:
            raise ValueError(f"No head update at seed {seed} U{row['update']}")
        if actor_delta["frozen_actor"]["max_abs_delta_from_bc"] != 0.0:
            raise ValueError(f"Frozen actor drift at seed {seed} U{row['update']}")
        if actor_delta["log_std_max_abs_delta_from_initial"] != 0.0:
    ...
    if status.get("status") == "COMPLETED":
        if status.get("last_completed_update") != 8 or len(metrics) != 8:
            raise ValueError(f"Completed seed {seed} is not exactly U1..U8")
        if [int(item["update"]) for item in checkpoints] != [8]:
...
def evaluation_log(seed: int | None = None) -> dict[str, str]:
    ...
    path = RUN_ROOT / "v1_3_d_logs" / filename
    if not path.is_file():
...
def build_report(results: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "V1.3-D is the infrastructure-stable replication of V1.3-C: the same physical-Gaussian "
        "PPO configuration with PyTorch autograd multithreading disabled.",
        "",
        "## Training process",
        "",
    ...
    if "baseline" in results:
        ...
        lines.extend([
            "## Fail-fast stop",
            "",
            f"Not started: `{results['not_started_seeds']}`. No BC or candidate evaluation was run.",
            "",
        ])
    ...
    lines.extend([
        ...
        "Canonical BC remains the deployment recommendation unless a later preregistered confirmation says otherwise.",
        "",
    ])
    return "\n".join(lines)
...
def main() -> None:
    ...
    if precheck.get("status") != "PASS" or preregistration.get("status") != "PREREGISTERED":
        raise ValueError("V1.3-D precheck/preregistration is not locked")
    ...
    if not training:
        raise ValueError("No formal V1.3-D run exists")

    results: dict[str, Any] = {
        "schema_version": 1,
        "record": "PPO_V1_3_D_RESULTS",
    ...
    if any(status == "STOPPED_KL_GUARDRAIL" for status in statuses):
        ...
        results["interpretation"] = (
            "The infrastructure-stable replication suffered a recorded runtime or native failure. "
            "Later seeds and all evaluations were stopped; no further recovery arm is authorized."
        ...
        results["interpretation"] = (
            "Training was finite, but at least one seed had fewer than 6/8 updates in the locked KL window. "
            "No development evaluation was consumed."
        )
    else:
    ...
    print(f"V1_3_D_VERDICT={results['final_verdict']}")
