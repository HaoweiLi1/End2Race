...
def _ttc_opp_setup(opp_poses, prev_oppo_pose, dt, opp_waypoints):
    """Opponent velocity and raceline projection for TTC."""
...
def get_obstacle_collision_mixed_ttc_obb(traj, traj_clothoid, v_lattice, opp_poses, prev_oppo_pose, dt,
    ...
    return 0.8 * static_cost_k + 0.2 * ttc_cost
...
def get_obstacle_collision_merged_ttc_obb(traj, traj_clothoid, v_lattice, opp_poses, prev_oppo_pose, dt,
    ...
    # OR fusion: either hit → full cost, no dilution
    return np.maximum(static_cost_k, ttc_cost)
