...
- phase: ego ahead of opponent (progress) means the collision happened
  post-overtake, otherwise pre-overtake.
...
def classify_episode(path, car_dist_thresh):
    ...
    phase = 'post' if ego_progress > opp_progress else 'pre'
...
def main():
    ...
    for path in episode_paths:
        cause, phase, final_dist = classify_episode(path, args.car_dist_thresh)
    ...
    header = f"{'':>12} {'pre-overtake':>14} {'post-overtake':>14}"
    print(header)
    for cause in CAUSES:
        row = [len(buckets[(cause, phase)]) for phase in PHASES]
        print(f"{cause:>12} {row[0]:>14} {row[1]:>14}")
    print()
