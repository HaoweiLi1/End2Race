...
> 当前工作状态与下一步请看 `CURRENT_HANDOFF.md`（唯一权威入口）。
> 仓库文件/代码结构说明请看 `docs/REPO_GUIDE.md`。
> 生成时间：2026-07-12。
...
**截至本文件生成时，这个目标从未被直接测量过。** 所有已完成的实验要么在优化代理指标，要么在验证中间产物。
...
**这些是本地唯一副本，远端没有备份**（07-08 起实验改为只在远端跑，之前的本地产物未上传）。处置方案见 `docs/CLEANUP_PROPOSAL.md`。
...
数据源头（P1 验证 NPZ）**只存在于远端**，本地完全没有——本地 33GB 的 `eval_results/` 与这条链**零重叠**。
...
- **唯一晋级门是真实闭环 KPI**：`fixed_collision > new_collision` 且 `gained_overtake >= lost_overtake`，TTC/Brier/报警**不得**参与排名。
...
**为什么三臂四项指标完全相同**：阈值取"第 8 大负例 episode max"，因此假介入**恒等于 7/75**（构造恒真，不携带任何臂的信息）；而三臂全部正例分数都落在各自阈值之下，recall 恒为 0。

**根因是正类饥饿，不是 sidecar 无效**：
...
- 三臂原始分数分布**确实不同**（正例 logit 带宽 A: 0.027 / B: 0.636 / C: 1.162）——B/C 学出了输入依赖，只是幅度远不够；**A 输出几乎是常数**（带宽 0.027），完全没学到输入依赖。
...
它已经消耗两天、约 4,600 行代码、两次失败，而它要解决的问题（PPO 探索不足）**有一个便宜得多的解法**：把介入门的初始先验从 logit −6（0.25%）调高到 −2（约 12%），让 PPO 自己从闭环反馈中学"何时该刹"。

而 warm-start 本身的任务是别扭的：它要求策略**只看当前观测**，就复现一个 **oracle 穷举 90 条分支、知道结局后**才选出的动作。这是"把搜索蒸馏进反应式策略"，出了名地吃数据，而教材只有 67 个 episode。

### 最关键的事实

**PPO 训练循环一行都还没写。** `bplus_v22/` 的 12,702 行里，宏动作、有界合成、多目标 buffer、变折扣 GAE、双 critic、超车 dual、词典序选择器、闭环评估器**全部已就绪**——唯独缺训练循环本身。

用户的目标（碰撞率、超车率）**至今未被直接测量过一次**。

### 下一步计划

见 `docs/superpowers/plans/2026-07-12-ppo-pilot-bc-direct.md`：写 PPO 训练循环，用**测量**（而非猜测）标定探索先验，从纯 BC 起步跑三臂 pilot，把"warm-start 是否必需"变成一个**可证伪的假设**，并首次直接测量真实 KPI。
