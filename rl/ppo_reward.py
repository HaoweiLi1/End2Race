"""PPO V1 progress projection and transition reward.

The evaluator and training environment share this module so that closed-track
progress, seam unwrapping, and collision accounting have one implementation.
"""
...
class PPOV1TransitionReward:
    """Stateful three-term PPO V1 reward for one environment instance."""
    ...
    def reset(self, raw_observation: dict[str, Any], *, scenario_id: str, ego_index: int = 0) -> None:
        ...
        if len(opponent_indices) != 1:
            raise ValueError(f"PPO V1 reward requires exactly one opponent, got {num_agents - 1}")
    ...
    def step(
        ...
        *,
        ...
        if self._previous_ego_progress is None or self._previous_opponent_progress is None:
            raise RuntimeError("PPO V1 reward must be reset before step")
        ...
        reward_progress = self.progress_weight * ego_delta
        reward_relative = 0.0 if self._opponent_collision_latched else self.relative_weight * (ego_delta - opponent_delta)
        if ego_collision and not self._ego_collision_penalty_applied:
            reward_collision = self.collision_penalty
            self._ego_collision_penalty_applied = True
        else:
            reward_collision = 0.0
        reward_total = reward_progress + reward_relative + reward_collision
        return PPOV1RewardResult(
